...
class ASPP(Model):
    def __init__(self, backbone, output_stride):
        ...
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool1d((1, 1)),
                                             nn.Conv1d(inplanes, 256, 1, stride=1, bias=False),
                                             BatchNorm(256),
                                             nn.ReLU())
        self.conv1 = nn.Conv1d(1280, 256, 1, bias=False)
        self.bn1 = nn.BatchNorm1d(256)
        self.relu = nn.ReLU()
        self.dropout = nn.BatchNorm1d(0.5)
        self._init_weight()
