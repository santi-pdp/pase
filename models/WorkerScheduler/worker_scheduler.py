...
class backprop_scheduler(object):
    ...
    def _drop_out(self, preds, label, cls_optim, regr_optim, frontend_optim, dropout_rate, device):
        ...
        re_mask = re_mask.from_numpy(re_mask).type(torch.FloatTensor).to(device).detach()
        cls_mask = cls_mask.from_numpy(cls_mask).type(torch.FloatTensor).to(device).detach()
